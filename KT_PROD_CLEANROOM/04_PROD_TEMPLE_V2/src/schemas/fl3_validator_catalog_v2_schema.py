...
FL3_VALIDATOR_CATALOG_V2_SCHEMA_ID = "kt.validator_catalog.v2"
FL3_VALIDATOR_CATALOG_V2_SCHEMA_FILE = "fl3/kt.validator_catalog.v2.json"
FL3_VALIDATOR_CATALOG_V2_SCHEMA_VERSION_HASH = schema_version_hash(FL3_VALIDATOR_CATALOG_V2_SCHEMA_FILE)
...
def _validate_validator_params(*, kind: str, params: Dict[str, Any]) -> Dict[str, Any]:
    enforce_max_fields(params, max_fields=32)
    validate_bounded_json_value(params, max_depth=10, max_string_len=4096, max_list_len=256)
    ...
    if k == "STRICT_JSON_OBJECT":
        require_keys(params, required={"required_keys", "spec", "no_extra_keys"})
        reject_unknown_keys(params, allowed={"required_keys", "spec", "no_extra_keys"})

        required_keys = ensure_sorted_str_list(params.get("required_keys"), field="required_keys")
        if len(required_keys) > 64:
            raise SchemaValidationError("STRICT_JSON_OBJECT required_keys too large (fail-closed)")

        spec = require_dict(params.get("spec"), name="spec")
        enforce_max_fields(spec, max_fields=64)
        validate_bounded_json_value(spec, max_depth=6, max_string_len=4096, max_list_len=0)

        allowed_prop_types = {"STRING", "NUMBER", "BOOL", "ARRAY_STRING"}
        spec_norm: Dict[str, Any] = {}
        for prop, raw in spec.items():
            key = str(prop).strip()
            if not key:
                raise SchemaValidationError("STRICT_JSON_OBJECT spec keys must be non-empty (fail-closed)")
            validate_short_string({"prop": key}, "prop", max_len=128)
            row = require_dict(raw, name=f"spec[{key}]")
            enforce_max_fields(row, max_fields=8)
            require_keys(row, required={"type"})
            ptype = str(row.get("type", "")).strip().upper()
            if ptype not in allowed_prop_types:
                raise SchemaValidationError("STRICT_JSON_OBJECT spec.type invalid (fail-closed)")

            if ptype == "STRING":
                reject_unknown_keys(row, allowed={"type", "min_len", "max_len"})
                mn = row.get("min_len", 0)
                mx = row.get("max_len", 4096)
                if not isinstance(mn, int) or mn < 0:
                    raise SchemaValidationError("STRICT_JSON_OBJECT STRING min_len invalid (fail-closed)")
                if not isinstance(mx, int) or mx < mn:
                    raise SchemaValidationError("STRICT_JSON_OBJECT STRING max_len invalid (fail-closed)")
                spec_norm[key] = {"type": "STRING", "min_len": int(mn), "max_len": int(mx)}

            elif ptype == "NUMBER":
                reject_unknown_keys(row, allowed={"type"})
                spec_norm[key] = {"type": "NUMBER"}

            elif ptype == "BOOL":
                reject_unknown_keys(row, allowed={"type"})
                spec_norm[key] = {"type": "BOOL"}

            elif ptype == "ARRAY_STRING":
                reject_unknown_keys(row, allowed={"type", "len_exact", "min_item_len", "max_item_len"})
                ln = row.get("len_exact")
                if not isinstance(ln, int) or ln < 0 or ln > 256:
                    raise SchemaValidationError("STRICT_JSON_OBJECT ARRAY_STRING len_exact invalid (fail-closed)")
                min_item_len = row.get("min_item_len", 0)
                max_item_len = row.get("max_item_len", 4096)
                if not isinstance(min_item_len, int) or min_item_len < 0:
                    raise SchemaValidationError("STRICT_JSON_OBJECT ARRAY_STRING min_item_len invalid (fail-closed)")
                if not isinstance(max_item_len, int) or max_item_len < min_item_len:
                    raise SchemaValidationError("STRICT_JSON_OBJECT ARRAY_STRING max_item_len invalid (fail-closed)")
                spec_norm[key] = {
                    "type": "ARRAY_STRING",
                    "len_exact": int(ln),
                    "min_item_len": int(min_item_len),
                    "max_item_len": int(max_item_len),
                }

        missing = [k for k in required_keys if k not in spec_norm]
        if missing:
            raise SchemaValidationError(f"STRICT_JSON_OBJECT spec missing required_keys: {missing!r} (fail-closed)")

        no_extra_keys = params.get("no_extra_keys")
        if not isinstance(no_extra_keys, bool):
            raise SchemaValidationError("STRICT_JSON_OBJECT no_extra_keys must be boolean (fail-closed)")

        params["required_keys"] = required_keys
        params["spec"] = {k: spec_norm[k] for k in sorted(spec_norm.keys())}
        params["no_extra_keys"] = bool(no_extra_keys)
        return params
...
def _validate_validators(value: Any) -> List[Dict[str, Any]]:
    ...
    for item in value:
        ...
        enforce_max_fields(row, max_fields=24)
...
def validate_fl3_validator_catalog_v2(obj: Dict[str, Any]) -> None:
    entry = require_dict(obj, name="validator_catalog_v2")
    enforce_max_fields(entry, max_fields=64)
    enforce_max_canonical_json_bytes(entry, max_bytes=256_000)
    require_keys(entry, required=_REQUIRED)
    reject_unknown_keys(entry, allowed=_ALLOWED)

    if entry.get("schema_id") != FL3_VALIDATOR_CATALOG_V2_SCHEMA_ID:
        raise SchemaValidationError("schema_id mismatch (fail-closed)")
    if entry.get("schema_version_hash") != FL3_VALIDATOR_CATALOG_V2_SCHEMA_VERSION_HASH:
