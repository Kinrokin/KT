...
FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_ID = "kt.audit_doctrine_proposal.v1"
FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_FILE = "fl3/kt.audit_doctrine_proposal.v1.json"
FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_VERSION_HASH = schema_version_hash(FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_FILE)
...
def validate_fl3_audit_doctrine_proposal(obj: Dict[str, Any]) -> None:
    entry = require_dict(obj, name="FL3 audit doctrine proposal")
    enforce_max_fields(entry, max_fields=24)
    require_keys(entry, required=_REQUIRED)
    reject_unknown_keys(entry, allowed=_ALLOWED)

    if entry.get("schema_id") != FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_ID:
        raise SchemaValidationError("schema_id mismatch (fail-closed)")
    if entry.get("schema_version_hash") != FL3_AUDIT_DOCTRINE_PROPOSAL_SCHEMA_VERSION_HASH:
