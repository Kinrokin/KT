...
def _iter_py_files(root: Path) -> Iterable[Path]:
    for p in root.rglob("*.py"):
        if any(part in {"__pycache__", "artifacts", "ledgers"} for part in p.parts):
            continue
        yield p
...
def check(root: Path) -> Tuple[bool, Sequence[Violation]]:
    ...
    # Python 3.10+ provides this; fail-closed if unavailable.
    ...
    third_party_allow = {"yaml", "psutil"}
    banned_prefixes = (
        # Runtime organ roots (must never be imported in the growth tool process)
        "kt",
        "core",
        "schemas",
        "memory",
