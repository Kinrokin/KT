...
def check(root: Path) -> Tuple[bool, Sequence[Violation]]:
    ...
    tooling_allow = {"crucible_loader", "crucible_dsl_schemas", "crucible_runner"}
    banned_prefixes = (
        "kt",
        "core",
        "schemas",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    root = Path(argv[0]).resolve() if argv else Path(__file__).resolve().parent / "orchestrator"
    ok, violations = check(root)
    if ok:
        print("# C018 CONSTITUTIONAL GUARD: PASS")
        print(f"- root: {root.as_posix()}")
        print(f"- files_scanned: {len(list(_iter_py_files(root)))}")
        return 0
    print("# C018 CONSTITUTIONAL GUARD: FAIL")
    for v in violations:
