...
def _is_allowed_import(
    *,
    ...
    stdlib: Set[str],
    third_party_allow: Set[str],
    tooling_allow: Set[str],
    banned_prefixes: Tuple[str, ...],
) -> bool:
    top = module.split(".", 1)[0]
    if module in tooling_allow:
        return True
...
def check(root: Path) -> Tuple[bool, Sequence[Violation]]:
    ...
    third_party_allow = {"yaml"}
    tooling_allow = {"curriculum_compiler", "teacher_schemas"}
    banned_prefixes = (
    ...
    for p in py_files:
        ...
        for lineno, kind, module in _extract_imports(tree):
            if not _is_allowed_import(
                ...
                local_modules=local,
                stdlib=stdlib,
                third_party_allow=third_party_allow,
                tooling_allow=tooling_allow,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    root = Path(argv[0]).resolve() if argv else Path(__file__).resolve().parent / "dream_loop"
    ok, violations = check(root)
    if ok:
        print("# C020 CONSTITUTIONAL GUARD: PASS")
        print(f"- root: {root.as_posix()}")
        print(f"- files_scanned: {len(list(_iter_py_files(root)))}")
        return 0
    print("# C020 CONSTITUTIONAL GUARD: FAIL")
    for v in violations:
