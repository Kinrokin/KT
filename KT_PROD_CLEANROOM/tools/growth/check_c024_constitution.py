...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    root = Path(argv[0]).resolve() if argv else Path(__file__).resolve().parent / "training_warehouse"
    ok, violations = check(root)
    if ok:
        print("# C024 CONSTITUTIONAL GUARD: PASS")
        print(f"- root: {root.as_posix()}")
        print(f"- files_scanned: {len(list(_iter_py_files(root)))}")
        return 0
    print("# C024 CONSTITUTIONAL GUARD: FAIL")
    for v in violations:
