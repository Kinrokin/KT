...
DEFAULT_FOURTH_RERUN_LAUNCH_CONTRACT_REL = "KT_PROD_CLEANROOM/governance/b04_r5_fourth_same_head_rerun_launch_contract.json"
DEFAULT_FOURTH_RERUN_LAUNCH_TERMINAL_STATE_REL = "KT_PROD_CLEANROOM/governance/b04_r5_fourth_same_head_rerun_terminal_state.json"
...
EXPECTED_FOURTH_RERUN_STEP_ID = "B04_R5_ROUTER_VS_BEST_ADAPTER_PROOF__FOURTH_SAME_HEAD_RERUN"
...
EXPECTED_FOURTH_RERUN_EXECUTION_MODE = "FOURTH_R5_RERUN_AUTHORIZED_ONLY__R6_STILL_BLOCKED_UNTIL_EARNED_SUPERIORITY"
EXPECTED_FOURTH_POST_RERUN_HOLD_EXECUTION_MODE = "R6_NEXT_IN_ORDER_BLOCKED_PENDING_EARNED_SUPERIORITY__FUTURE_FOURTH_R5_RERUN_REQUIRES_NEW_LAB_JUSTIFICATION"
...
def _r5_execution_context(
    *,
    ...
    second_rerun_path = (
        next_step == EXPECTED_SECOND_RERUN_STEP_ID
        and execution_mode == EXPECTED_SECOND_RERUN_EXECUTION_MODE
        and overlay_step == EXPECTED_SECOND_RERUN_STEP_ID
    ...
    fourth_rerun_path = (
        next_step == EXPECTED_FOURTH_RERUN_STEP_ID
        and execution_mode == EXPECTED_FOURTH_RERUN_EXECUTION_MODE
        and overlay_step == EXPECTED_FOURTH_RERUN_STEP_ID
    ...
    second_post_rerun_hold_path = (
        source_step == EXPECTED_SECOND_RERUN_STEP_ID
        and next_step == EXPECTED_EARNED_NEXT_STEP_ID
        and execution_mode == EXPECTED_SECOND_POST_RERUN_HOLD_EXECUTION_MODE
        and overlay_step == EXPECTED_EARNED_NEXT_STEP_ID
        and overlay_batch == EXPECTED_SECOND_RERUN_STEP_ID
        and overlay_workstream == EXPECTED_SECOND_RERUN_STEP_ID
        and resume_step == EXPECTED_EARNED_NEXT_STEP_ID
        and resume_workstream == EXPECTED_SECOND_RERUN_STEP_ID
        and reanchor_step == EXPECTED_HOLD_NEXT_STEP_ID
        and reanchor_workstream == EXPECTED_SECOND_RERUN_STEP_ID
        and bool(next_contract.get("repo_state_executable_now")) is False
        and bool(resume.get("repo_state_executable_now")) is False
    )
    ...
    fourth_post_rerun_hold_path = (
        source_step == EXPECTED_FOURTH_RERUN_STEP_ID
        and next_step == EXPECTED_EARNED_NEXT_STEP_ID
        and execution_mode == EXPECTED_FOURTH_POST_RERUN_HOLD_EXECUTION_MODE
        and overlay_step == EXPECTED_EARNED_NEXT_STEP_ID
        and overlay_batch == EXPECTED_FOURTH_RERUN_STEP_ID
        and overlay_workstream == EXPECTED_FOURTH_RERUN_STEP_ID
        and resume_step == EXPECTED_EARNED_NEXT_STEP_ID
        and resume_workstream == EXPECTED_FOURTH_RERUN_STEP_ID
        and reanchor_step == EXPECTED_HOLD_NEXT_STEP_ID
        and reanchor_workstream == EXPECTED_FOURTH_RERUN_STEP_ID
        and bool(next_contract.get("repo_state_executable_now")) is False
        and bool(resume.get("repo_state_executable_now")) is False
    )
    ...
    if third_post_rerun_hold_path:
        return EXPECTED_THIRD_RERUN_STEP_ID
    if fourth_post_rerun_hold_path:
        return EXPECTED_FOURTH_RERUN_STEP_ID
    return None
...
def build_router_vs_best_adapter_proof_ratification_receipt(
    *,
    ...
    fourth_rerun_launch_contract = load_json(root / DEFAULT_FOURTH_RERUN_LAUNCH_CONTRACT_REL)
    fourth_rerun_launch_terminal = load_json(root / DEFAULT_FOURTH_RERUN_LAUNCH_TERMINAL_STATE_REL)
    ...
    fourth_rerun_context_active = execution_context == EXPECTED_FOURTH_RERUN_STEP_ID
    ...
    second_rerun_launch_surface_honest = (
        str(second_rerun_launch_contract.get("workstream_id", "")).strip() == "B04_R5_SECOND_SAME_HEAD_RERUN_LAUNCH_SURFACE"
        and str(second_rerun_launch_contract.get("launch_mode", "")).strip()
        == "COUNTED_SECOND_R5_RERUN_AUTHORIZATION_ONLY_NO_R6_OR_HIGHER_WIDENING"
        and second_rerun_launch_terminal.get("counted_r5_second_rerun_authorized") is True
        and second_rerun_launch_terminal.get("learned_router_authorized") is False
        and second_rerun_launch_terminal.get("lobe_ratified") is False
        and (
            (
                second_rerun_launch_terminal.get("counted_r5_second_rerun_executed") is False
                and str(second_rerun_launch_terminal.get("current_state", "")).strip() == "B04_R5_SECOND_SAME_HEAD_RERUN_AUTHORIZED_NOT_YET_EXECUTED"
                and str(second_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_SECOND_RERUN_STEP_ID
            )
            or (
                second_rerun_launch_terminal.get("counted_r5_second_rerun_executed") is True
                and (
                    (
                        second_rerun_launch_terminal.get("router_superiority_earned") is False
                        and str(second_rerun_launch_terminal.get("current_state", "")).strip() == "B04_R5_SECOND_SAME_HEAD_RERUN_EXECUTED_STATIC_HOLD"
                        and str(second_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_HOLD_NEXT_STEP_ID
                    )
                    or (
                        second_rerun_launch_terminal.get("router_superiority_earned") is True
                        and str(second_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_EARNED_NEXT_STEP_ID
                    )
                )
            )
        )
    ...
    fourth_rerun_launch_surface_honest = (
        str(fourth_rerun_launch_contract.get("workstream_id", "")).strip() == "B04_R5_FOURTH_SAME_HEAD_RERUN_LAUNCH_SURFACE"
        and str(fourth_rerun_launch_contract.get("launch_mode", "")).strip()
        == "COUNTED_FOURTH_R5_RERUN_AUTHORIZATION_ONLY_NO_R6_OR_HIGHER_WIDENING"
        and fourth_rerun_launch_terminal.get("counted_r5_fourth_rerun_authorized") is True
        and fourth_rerun_launch_terminal.get("learned_router_authorized") is False
        and fourth_rerun_launch_terminal.get("lobe_ratified") is False
        and (
            (
                fourth_rerun_launch_terminal.get("counted_r5_fourth_rerun_executed") is False
                and str(fourth_rerun_launch_terminal.get("current_state", "")).strip() == "B04_R5_FOURTH_SAME_HEAD_RERUN_AUTHORIZED_NOT_YET_EXECUTED"
                and str(fourth_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_FOURTH_RERUN_STEP_ID
            )
            or (
                fourth_rerun_launch_terminal.get("counted_r5_fourth_rerun_executed") is True
                and (
                    (
                        fourth_rerun_launch_terminal.get("router_superiority_earned") is False
                        and str(fourth_rerun_launch_terminal.get("current_state", "")).strip() == "B04_R5_FOURTH_SAME_HEAD_RERUN_EXECUTED_STATIC_HOLD"
                        and str(fourth_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_HOLD_NEXT_STEP_ID
                    )
                    or (
                        fourth_rerun_launch_terminal.get("router_superiority_earned") is True
                        and str(fourth_rerun_launch_terminal.get("next_lawful_move", "")).strip() == EXPECTED_EARNED_NEXT_STEP_ID
                    )
                )
            )
        )
    ...
    checks = [
        ...
        },
        {
            ...
            "pass": (
                (not rerun_context_active and not second_rerun_context_active and not third_rerun_context_active and not fourth_rerun_context_active)
                ...
                or (fourth_rerun_context_active and fourth_rerun_launch_surface_honest)
            ),
        },
        {
