...
OUTPUT_PACKET = "cohort0_successor_gate_d_narrow_admissibility_review_packet.json"
OUTPUT_RECEIPT = "cohort0_successor_gate_d_narrow_admissibility_review_receipt.json"
OUTPUT_REPORT = "COHORT0_SUCCESSOR_GATE_D_NARROW_ADMISSIBILITY_REVIEW_REPORT.md"

OUTCOME_CONFIRMED = "NARROW_ADMISSIBILITY_CONFIRMED__LIMITED_REVIEW_POSTURE_ONLY"
OUTCOME_DEFERRED = "NARROW_ADMISSIBILITY_DEFERRED__STRENGTHENING_REQUIRED"
OUTCOME_DENIED = "NARROW_ADMISSIBILITY_DENIED__BOUNDED_DEFECT_IDENTIFIED"
NEXT_LAWFUL_MOVE = "MAINTAIN_NARROW_SUCCESSOR_GATE_D_ADMISSIBILITY_POSTURE__STILL_PRE_GATE_D"
...
def _require_same_subject_head(packets: Sequence[Dict[str, Any]]) -> str:
    ...
    if len(heads) != 1:
        raise RuntimeError("FAIL_CLOSED: narrow admissibility review requires one same-head authority line")
...
def _validate_inputs(
    *,
    ...
    admissibility_packet: Dict[str, Any],
    ...
    for payload, label in (
        ...
        (admissibility_packet, "successor gate d reentry admissibility screen packet"),
        (admissibility_receipt, "successor gate d reentry admissibility screen receipt"),
    ):
    ...
    if str(prep_receipt.get("execution_status", "")).strip() != "PASS__SUCCESSOR_REENTRY_PREP_PACKET_AUTHORED":
        raise RuntimeError("FAIL_CLOSED: successor reentry-prep packet must exist before narrow review")
    if str(admissibility_receipt.get("execution_status", "")).strip() != "PASS__SUCCESSOR_GATE_D_REENTRY_ADMISSIBILITY_SCREEN_EXECUTED":
        raise RuntimeError("FAIL_CLOSED: successor admissibility screen must exist before narrow review")
    if not bool(admissibility_receipt.get("narrow_successor_gate_d_admissibility_review_authorized", False)):
        raise RuntimeError("FAIL_CLOSED: narrow admissibility review must be authorized before convening")
    if str(admissibility_receipt.get("next_lawful_move", "")).strip() != admissibility_screen.NEXT_LAWFUL_MOVE:
        raise RuntimeError("FAIL_CLOSED: narrow admissibility review next move mismatch")

    for receipt in (prep_receipt, admissibility_receipt):
        if bool(receipt.get("same_head_counted_reentry_admissible_now", True)):
            raise RuntimeError("FAIL_CLOSED: counted reentry must remain blocked entering narrow admissibility review")
        if bool(receipt.get("gate_d_reopened", True)):
            raise RuntimeError("FAIL_CLOSED: Gate D must remain closed entering narrow admissibility review")
        if bool(receipt.get("gate_e_open", True)):
            raise RuntimeError("FAIL_CLOSED: Gate E must remain closed entering narrow admissibility review")


def _determine_outcome(*, findings: Dict[str, Any], admissibility_authorized: bool) -> str:
    if not admissibility_authorized:
        return OUTCOME_DEFERRED
    if not bool(findings.get("selected_bridge_cross_lane_hold", False)):
        return OUTCOME_DENIED
    if not bool(findings.get("route_consequence_cross_lane_nonzero", False)):
        return OUTCOME_DENIED
    if not bool(findings.get("dominance_broadening_visible", False)):
        return OUTCOME_DEFERRED
    if not bool(findings.get("materially_distinct_family_lane_executed", False)):
        return OUTCOME_DENIED
    if not bool(findings.get("reserve_challenges_pass", False)):
        return OUTCOME_DENIED
    if not bool(findings.get("fixed_harness_stable", False)):
        return OUTCOME_DENIED
...
def _build_outputs(
    *,
    ...
    findings = dict(admissibility_packet.get("screen_findings", {}))
    authorized = bool(admissibility_packet.get("narrow_successor_gate_d_admissibility_review_authorized", False))
    outcome = _determine_outcome(findings=findings, admissibility_authorized=authorized)

    bounded_defects_remaining: list[str] = []
    if outcome == OUTCOME_DEFERRED:
        bounded_defects_remaining.append("narrow_admissibility_not_yet_confirmed")
    if outcome == OUTCOME_DENIED:
        for key in (
            "selected_bridge_cross_lane_hold",
            "route_consequence_cross_lane_nonzero",
            "dominance_broadening_visible",
            "materially_distinct_family_lane_executed",
            "reserve_challenges_pass",
            "fixed_harness_stable",
        ):
            if not bool(findings.get(key, False)):
                bounded_defects_remaining.append(key)

    packet = {
        "schema_id": "kt.operator.cohort0_successor_gate_d_narrow_admissibility_review_packet.v1",
        "status": "PASS",
        "generated_utc": utc_now_iso_z(),
        "claim_boundary": (
            "This review decides only whether the currently bound successor evidence bundle supports a narrow successor Gate D "
            "admissibility posture, still strictly pre-Gate-D. It does not reopen Gate D, authorize counted reentry, "
            "or open Gate E."
        ),
        ...
        "review_basis": {
            "selected_successor_core": dict(prep_packet.get("selected_successor_core", {})),
            "proofs_established": list(prep_packet.get("proofs_established", [])),
            "non_claim_boundary_section": dict(prep_packet.get("non_claim_boundary_section", {})),
        },
        "bounded_defects_remaining": bounded_defects_remaining,
        "narrow_successor_gate_d_admissibility_confirmed": outcome == OUTCOME_CONFIRMED,
        ...
        "next_lawful_move": (
            ...
            else "RUN_ADDITIONAL_STRENGTHENING_BEFORE_RECONVENING_NARROW_ADMISSIBILITY_REVIEW"
        ),
    ...
    receipt = {
        "schema_id": "kt.operator.cohort0_successor_gate_d_narrow_admissibility_review_receipt.v1",
        "status": "PASS",
        "generated_utc": utc_now_iso_z(),
        ...
        "review_outcome": outcome,
        "narrow_successor_gate_d_admissibility_confirmed": packet["narrow_successor_gate_d_admissibility_confirmed"],
        "broad_same_head_counted_reentry_block_remains": True,
        "full_successor_gate_d_readjudication_authorized_now": False,
        "same_head_counted_reentry_admissible_now": False,
        "gate_d_reopened": False,
...
def _build_report(*, packet: Dict[str, Any], receipt: Dict[str, Any]) -> str:
    findings = dict(packet.get("review_findings", {}))
    return (
        "# Cohort0 Successor Gate D Narrow Admissibility Review Report\n\n"
        f"- Execution status: `{receipt.get('execution_status', '')}`\n"
        f"- Review scope: `{receipt.get('review_scope', '')}`\n"
        f"- Review outcome: `{receipt.get('review_outcome', '')}`\n"
        f"- Narrow admissibility confirmed: `{receipt.get('narrow_successor_gate_d_admissibility_confirmed', False)}`\n"
...
def run(
    *,
    ...
    admissibility_packet_path: Path,
    ...
    admissibility_packet = _load_json_required(
        admissibility_packet_path, label="successor gate d reentry admissibility screen packet"
    )
    admissibility_receipt = _load_json_required(
        admissibility_receipt_path, label="successor gate d reentry admissibility screen receipt"
    )
    ...
    _validate_inputs(
        ...
        admissibility_packet=admissibility_packet,
    ...
    source_refs = {
        ...
        "successor_gate_d_reentry_admissibility_screen_packet_ref": admissibility_packet_path.as_posix(),
        "successor_gate_d_reentry_admissibility_screen_receipt_ref": admissibility_receipt_path.as_posix(),
    }
    ...
    return {
        ...
        "review_outcome": outputs["receipt"]["review_outcome"],
        "narrow_successor_gate_d_admissibility_confirmed": outputs["receipt"][
            "narrow_successor_gate_d_admissibility_confirmed"
        ],
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Convene the narrow successor Gate D admissibility review, still strictly pre-Gate-D."
    )
    ...
    parser.add_argument("--admissibility-packet", default=DEFAULT_ADMISSIBILITY_SCREEN_PACKET_REL)
    parser.add_argument("--admissibility-receipt", default=DEFAULT_ADMISSIBILITY_SCREEN_RECEIPT_REL)
    ...
    result = run(
        ...
        admissibility_packet_path=_resolve(root, args.admissibility_packet),
        admissibility_receipt_path=_resolve(root, args.admissibility_receipt),
