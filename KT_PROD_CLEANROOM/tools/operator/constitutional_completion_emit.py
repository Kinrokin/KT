...
def _domain6_outputs(root: Path, head_sha: str) -> Dict[str, Dict[str, Any]]:
    ...
    outputs: Dict[str, Dict[str, Any]] = {
        ...
        "KT_PROD_CLEANROOM/governance/public_verifier_rules.json": _law_surface(
            schema_id="kt.governance.public_verifier_rules.v1",
            law_id="PUBLIC_VERIFIER_RULES_V1_20260310",
            domain_id="DOMAIN_6_EXTERNAL_LEGIBILITY",
            summary="Public verifier surfaces expose enough truth to verify posture without exposing private crown-jewel state.",
            authorities=[TRUTH_POINTER_REF, "KT_PROD_CLEANROOM/reports/current_state_receipt.json"],
            rules=["Public verifier manifests may reference authoritative pointers, state receipts, and branch protection evidence only."],
            validators=["python -m tools.operator.constitutional_completion_emit"],
        ),
        ...
        "KT_PROD_CLEANROOM/reports/public_verifier_manifest.json": {
            "schema_id": "kt.public_verifier_manifest.v1",
            "generated_utc": utc_now_iso_z(),
            "status": "PASS",
            "validated_head_sha": head_sha,
