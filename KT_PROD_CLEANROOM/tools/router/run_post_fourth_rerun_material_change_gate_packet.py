...
def _validate_candidate_refresh(packet: Dict[str, Any]) -> None:
    if str(packet.get("schema_id", "")).strip() != "kt.fifth_terminal_lab_readiness_refresh_packet.v1":
        raise RuntimeError("FAIL_CLOSED: fifth terminal refresh packet schema mismatch")
    if str(packet.get("mode", "")).strip() != "LAB_ONLY_NONCANONICAL":
        raise RuntimeError("FAIL_CLOSED: fifth terminal refresh packet mode mismatch")
    if str(packet.get("status", "")).strip() != "PASS":
        raise RuntimeError("FAIL_CLOSED: fifth terminal refresh packet must be PASS")
...
def build_post_fourth_rerun_material_change_gate_packet(
    *,
    ...
    candidate_primary_summary = candidate_refresh_packet.get("primary_candidate_summary")
    ...
    if not isinstance(candidate_primary_summary, dict):
        raise RuntimeError("FAIL_CLOSED: candidate primary summary missing")
    ...
    primary_candidate_adapter = str(candidate_primary_summary.get("adapter_id", "")).strip()
    primary_candidate_route_pattern = str(candidate_primary_summary.get("recommended_route_pattern", "")).strip()
    ...
    candidate_core_constraints_confirmed = all(
        ...
        for key in (
            ...
            "fifth_terminal_diversity_visible",
            "primary_candidate_terminal_visible",
            "primary_candidate_route_pattern_visible",
            "expanded_beyond_frozen_four",
        )
    )
    ...
    primary_candidate_is_new_terminal = primary_candidate_adapter in new_terminal_adapters
    primary_candidate_route_pair_is_new = primary_candidate_route_pattern in new_route_pairs
    ...
    material_change_earned = (
        ...
        and primary_candidate_is_new_terminal
        and primary_candidate_route_pair_is_new
    )
    semantic_bypass_risk = not material_change_earned and same_preserved_ceiling_story_under_new_label

    blockers: List[str] = []
    if same_head_as_ceiling:
        blockers.append("CANDIDATE_REFRESH_IS_SAME_LAB_HEAD_AS_POST_FOURTH_RERUN_CEILING")
    ...
    if not introduced_new_terminal_adapter:
        blockers.append("NO_NEW_TERMINAL_ADAPTER_BEYOND_POST_FOURTH_RERUN_CEILING")
    if not expanded_terminal_adapter_count:
        blockers.append("TERMINAL_ADAPTER_COUNT_NOT_EXPANDED_BEYOND_POST_FOURTH_RERUN_CEILING")
    if not introduced_new_route_pair:
        blockers.append("NO_NEW_ROUTE_PAIR_BEYOND_POST_FOURTH_RERUN_CEILING")
    if not primary_candidate_is_new_terminal:
        blockers.append("PRIMARY_CANDIDATE_NOT_ACTING_AS_NEW_TERMINAL")
    if not primary_candidate_route_pair_is_new:
        blockers.append("PRIMARY_CANDIDATE_ROUTE_PATTERN_NOT_NEW_BEYOND_CEILING")
    if semantic_bypass_risk:
        blockers.append("SAME_PRESERVED_POST_FOURTH_RERUN_CEILING_STORY__NOT_MATERIAL_CHANGE")

    posture = (
        "READY_FOR_POST_FOURTH_RERUN_ROUTER_READINESS_RECONSIDERATION_INPUT_CONSIDERATION"
        if material_change_earned
        else "HOLD_LAB_ONLY_PENDING_POST_FOURTH_RERUN_MATERIAL_CHANGE"
    )

    return {
        "schema_id": "kt.post_fourth_rerun_material_change_gate_packet.v1",
        ...
        "claim_boundary": (
            ...
            "does not earn router superiority, and cannot unlock R6. Its only job is to say whether a new post-fourth-rerun "
            "lab head is materially different enough from the preserved four-terminal reconsideration ceiling to justify "
            "preparing a later reconsideration input again."
        ),
        "questions": {
            "same_head_as_post_fourth_rerun_ceiling": same_head_as_ceiling,
            ...
            "primary_candidate_is_new_terminal": primary_candidate_is_new_terminal,
            "primary_candidate_route_pair_is_new": primary_candidate_route_pair_is_new,
            "same_terminal_adapter_set_as_post_fourth_rerun_ceiling": same_terminal_adapter_set_as_ceiling,
            "same_route_pair_set_as_post_fourth_rerun_ceiling": same_route_pair_set_as_ceiling,
            "same_preserved_post_fourth_rerun_ceiling_story_under_new_label": same_preserved_ceiling_story_under_new_label,
        ...
        "later_reconsideration_gate": (
            "Only when material_change_earned = true may a later post-fourth-rerun router-readiness reconsideration input be prepared. "
        ...
        "candidate_summary": {
            ...
            "primary_candidate_adapter_id": primary_candidate_adapter,
            "primary_candidate_route_pattern": primary_candidate_route_pattern,
        },
    }
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Gate a new post-fourth-rerun router material-change candidate against the preserved four-terminal reconsideration ceiling."
    )
