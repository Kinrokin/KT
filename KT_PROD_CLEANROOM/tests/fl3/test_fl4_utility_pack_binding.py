...
def _mk_jobspec(*, export_shadow_root: str, export_promoted_root: str) -> dict:
    job = {
        ...
        "job_id": "",
        "adapter_id": "fl4.utility_pack.binding.v1",
        "adapter_version": "1",
        "role": "ARCHITECT",
        "mode": "SMOKE",
        "run_kind": "STANDARD",
...
def test_eval_report_utility_pack_hash_mismatch_fails_closed(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Corrupt eval_report utility pack hash binding.
        er_path = job_dir / "eval_report.json"
        eval_report = json.loads(er_path.read_text(encoding="utf-8"))
        eval_report["utility_pack_hash"] = "0" * 64
        er_path.write_text(json.dumps(eval_report, indent=2, sort_keys=True, ensure_ascii=True) + "\n", encoding="utf-8")
