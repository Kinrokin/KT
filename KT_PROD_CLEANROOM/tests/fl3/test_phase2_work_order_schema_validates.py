...
def test_phase2_work_order_schema_validates_known_good() -> None:
    p = _REPO_ROOT / "KT_PROD_CLEANROOM" / "kt.phase2_work_order.v1.json"
    obj = loads_no_dupes(p.read_text(encoding="utf-8"))
    validate_object_with_binding(obj)


def test_phase2_work_order_schema_rejects_missing_constraints() -> None:
    p = _REPO_ROOT / "KT_PROD_CLEANROOM" / "kt.phase2_work_order.v1.json"
    obj = loads_no_dupes(p.read_text(encoding="utf-8"))
...
def test_phase2_work_order_rejects_duplicate_keys() -> None:
    with pytest.raises(DuplicateKeyError):
        loads_no_dupes('{"schema_id":"kt.phase2_work_order.v1","schema_id":"kt.phase2_work_order.v1"}')
