...
from tools.operator import cohort0_single_axis_crucible_input_tranche as tranche
...
def test_single_axis_crucible_input_tranche_binds_all_families_and_controls(tmp_path: Path) -> None:
    ...
    _write_json(residual_packet, {"status": "PASS", "subject_head": subject_head, "next_lawful_move": "AUTHOR_SINGLE_AXIS_CRUCIBLE_INPUTS_AND_EXECUTE_LAB_ONLY_SWEEPS"})
    ...
    _write_json(
        alpha_liability_registry,
        {
            ...
            "subject_head": subject_head,
            "rows": [
                {"family_id": row["family_id"], "target_lobe_id": row["target_lobe_id"], "alpha_should_lose_here_because": f"{row['family_id']} because", "acceptance_metric": row["acceptance_metric"], "expected_route_outcome": "ROUTE_TO_SPECIALIST", "new_admissible_eval_family": row["new_admissible_eval_family"]}
    ...
    payload = tranche.run_single_axis_crucible_input_tranche(
    ...
    manifest = payload["single_axis_crucible_input_manifest"]
    transfer = payload["single_axis_transfer_candidates"]

    assert receipt["status"] == "PASS"
    assert receipt["single_axis_posture"] == tranche.POSTURE
    assert receipt["next_lawful_move"] == tranche.NEXT_MOVE
    assert len(manifest["family_rows"]) == 8
    assert manifest["control_family_ids"] == ["BOUNDARY_ABSTENTION_CONTROL", "STATIC_NO_ROUTE_CONTROL"]
    assert any(row["transfer_candidate_status"] == "CONTROL_ONLY_NOT_TRANSFER_CANDIDATE" for row in transfer["rows"])
    assert any(row["transfer_candidate_status"] == "PENDING_SINGLE_AXIS_EXECUTION" for row in transfer["rows"])

    tracked = json.loads((reports_root / "single_axis_crucible_receipt.json").read_text(encoding="utf-8"))
    assert tracked["carrier_surface_role"] == "TRACKED_CARRIER_ONLY_SINGLE_AXIS_CRUCIBLE_RECEIPT"
