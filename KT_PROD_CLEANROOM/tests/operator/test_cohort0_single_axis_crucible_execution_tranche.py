...
def _build_authored_inputs(tmp_path: Path) -> dict[str, Path]:
    reports_root = tmp_path / "reports"
    authoritative_root = tmp_path / "authoritative_inputs"
...
def test_single_axis_execution_tranche_binds_survivors_and_preserves_controls(tmp_path: Path) -> None:
    authored = _build_authored_inputs(tmp_path)
    reports_root = authored["reports_root"]
    authoritative_root = tmp_path / "authoritative_execution"

    payload = execution.run_single_axis_crucible_execution_tranche(
        ...
        transfer_guard_path=authored["transfer_guard"],
        verdict_grammar_path=authored["verdict_grammar"],
        authoritative_root=authoritative_root,
        reports_root=reports_root,
        workspace_root=ROOT,
    )
    ...
    transfer = payload["single_axis_transfer_eligibility"]
    controls = payload["single_axis_control_validation"]

    assert receipt["status"] == "PASS"
    assert receipt["single_axis_execution_posture"] == execution.POSTURE
    assert receipt["next_lawful_move"] == execution.NEXT_MOVE_SURVIVORS
    assert receipt["survivor_family_count"] >= 4
    assert "CHILD_ANOMALY_PRESERVATION" in receipt["blocked_family_ids"]
    assert controls["controls_preserved"] is True
    assert "P2_SIGNAL_NOISE_SEPARATION" in transfer["survivor_family_ids"]
    assert any(
        row["family_id"] == "BOUNDARY_ABSTENTION_CONTROL" and row["transfer_candidate_status"] == "CONTROL_ONLY_NOT_TRANSFER_CANDIDATE"
        for row in transfer["rows"]
    )

    tracked = json.loads((reports_root / "single_axis_crucible_execution_receipt.json").read_text(encoding="utf-8"))
    assert tracked["carrier_surface_role"] == "TRACKED_CARRIER_ONLY_SINGLE_AXIS_CRUCIBLE_EXECUTION_RECEIPT"
