...
MAIN_HEAD = "e707efe9296dfd813e7f270305699bbb7b4819fd"
...
def validation_outputs(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Path]:
    tmp_path = tmp_path_factory.mktemp("commercial_proof_plane_validation")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_validation_selects_adversarial_proof_corridor_next(validation_outputs: Path) -> None:
    receipt = _payload(validation_outputs, "validation_receipt")
    assert receipt["selected_outcome"] == validator.SELECTED_OUTCOME
    assert receipt["next_lawful_move"] == "AUTHOR_KT_ADVERSARIAL_PROOF_CORRIDOR_SUPERLANE_V1"
    assert receipt["commercial_proof_plane_validated"] is True
    assert receipt["adversarial_proof_corridor_next"] is True
    assert receipt["adversarial_proof_corridor_authorized"] is False


def test_validation_preserves_boundaries(validation_outputs: Path) -> None:
    decision = _payload(validation_outputs, "adversarial_proof_corridor_gate_decision")
    assert decision["adversarial_proof_corridor_next"] is True
    assert decision["adversarial_proof_corridor_authorized"] is False
...
def test_rejects_packet_outcome_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["packet_receipt"]
    payload = _load(path)
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_OUTCOME_DRIFT"


def test_rejects_packet_next_move_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["next_lawful_move"]
    payload = _load(path)
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_NEXT_MOVE_DRIFT"
...
def test_rejects_bound_source_hash_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_SOURCE_HASH_MISMATCH"


def test_rejects_missing_deliverable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    (tmp_path / plane.OUTPUTS["quickstart"]).unlink()
    _patch_env(monkeypatch, tmp_path)
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_DELIVERABLE_MISSING"


def test_rejects_public_verifier_manifest_unbound(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["source_manifest"]
    payload = _load(path)
    payload["sources"] = [row for row in payload["sources"] if row.get("path") != plane.SOURCE_INPUTS["public_verifier_manifest"]]
    _write(path, payload)
    _patch_env(monkeypatch, tmp_path)
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
...
def test_rejects_premature_commercial_proof_validation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    path = tmp_path / plane.OUTPUTS["packet_receipt"]
    payload = _load(path)
    payload["commercial_proof_plane_validated"] = True
    _write(path, payload)
    _patch_env(monkeypatch, tmp_path)
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_PREMATURE_AUTHORITY"


def test_rejects_commercial_activation_claim_authorization(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["claim_boundary_receipt"]
    payload = _load(path)
    payload["commercial_activation_claim_authorized"] = True
    _write(path, payload)
    _patch_env(monkeypatch, tmp_path)
...
def test_rejects_external_audit_completion_claim(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["security_review_packet"]
    payload = _load(path)
...
def test_rejects_7b_amplification_claim(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["claim_boundary_receipt"]
    payload = _load(path)
...
def test_rejects_beyond_sota_claim_in_generated_markdown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["customer_safe_language"]
    path.write_text(path.read_text(encoding="utf-8") + "\nBeyond-SOTA is proven.\n", encoding="utf-8")
    _patch_env(monkeypatch, tmp_path)
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_CLAIM_BOUNDARY_BREACH"


def test_rejects_fp0_or_highway_authority_promotion(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["claim_boundary_receipt"]
    payload = _load(path)
    payload["fp0_or_highway_promoted_to_authority"] = True
    _write(path, payload)
    _patch_env(monkeypatch, tmp_path)
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
...
def test_rejects_duplicate_reason_codes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _copy_inputs(tmp_path)
    path = tmp_path / plane.OUTPUTS["validation_reason_codes"]
    payload = _load(path)
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_REASON_CODE_DUPLICATE"
...
def test_rejects_unexpected_branch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_BRANCH_DRIFT"
...
def test_rejects_trust_zone_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validator.LaneFailure) as excinfo:
        validator.run(output_root=tmp_path)
    assert excinfo.value.code == "RC_KT_COMMERCIAL_PROOF_PLANE_VAL_TRUST_ZONE_FAILED"
