...
EXEC_HEAD = "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
EXEC_MAIN_HEAD = "19b5e4c2e37a32910025afcde9725ee912d19521"


def _load_validation_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_package_promotion_authorization_packet_validation.py")
    spec = importlib.util.spec_from_file_location("b04_r6_package_auth_validation_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load package authorization validation helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("package_promotion_execution_packet")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_execution_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
    ...
    if filename.endswith(".md"):
        ...
        assert "package promotion execution packet" in text
        assert "does not run package promotion" in text
    else:
        payload = _load(path)
...
def test_authorization_validation_execution_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(execution.LaneFailure) as excinfo:
        execution.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_PACKAGE_PROMOTION_EXEC_PACKET_EXECUTION_DRIFT"
...
def test_claim_token_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(execution.LaneFailure) as excinfo:
        execution.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_PACKAGE_PROMOTION_EXEC_PACKET_CLAIM_TOKEN_DRIFT"
...
def test_main_replay_requires_head_equal_origin_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(execution.LaneFailure) as excinfo:
        execution.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_PACKAGE_PROMOTION_EXEC_PACKET_NEXT_MOVE_DRIFT"
