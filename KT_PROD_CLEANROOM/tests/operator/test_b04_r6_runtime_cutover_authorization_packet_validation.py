...
from tools.operator import cohort0_b04_r6_runtime_cutover_authorization_packet_validation as validation
from tools.operator.titanium_common import file_sha256


VALIDATION_HEAD = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
VALIDATION_MAIN_HEAD = "82cf0ac8d8d5870ba237eaede452ab10bb85c14e"


def _load_auth_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_runtime_cutover_authorization_packet.py")
    spec = importlib.util.spec_from_file_location("b04_r6_runtime_cutover_authorization_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load runtime cutover authorization helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("runtime_cutover_authorization_validation")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_runtime_cutover_authorization_validation_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
    path = outputs / filename
    assert path.exists()
    if filename.endswith(".md"):
        assert "does not execute runtime cutover" in path.read_text(encoding="utf-8").lower()
    else:
        payload = _load(path)
...
def test_validation_binds_runtime_cutover_authorization_packet(outputs: Path) -> None:
...
def test_next_lawful_move_is_runtime_cutover_execution_packet_authoring(outputs: Path) -> None:
    nxt = _next(outputs)
...
def test_validation_marks_authorization_validated_but_no_execution(outputs: Path) -> None:
    contract = _contract(outputs)
    assert contract["runtime_cutover_authorization_packet_authored"] is True
    assert contract["runtime_cutover_authorization_validated"] is True
    assert contract["runtime_cutover_execution_packet_authored"] is False
    assert contract["runtime_cutover_execution_packet_validated"] is False
    assert contract["runtime_cutover_authorized"] is False
    assert contract["runtime_cutover_executed"] is False
...
def test_authorization_operational_contracts_stay_bound_non_executing(outputs: Path, role: str) -> None:
    ...
    assert payload["runtime_cutover_executed"] is False
...
def test_validation_prep_only_outputs_remain_prep_only(outputs: Path, role: str) -> None:
    ...
    assert payload["cannot_execute_runtime_cutover"] is True
...
def test_runtime_cutover_authorization_validated_does_not_mean_cutover_authorized(outputs: Path) -> None:
    contract = _contract(outputs)
    assert contract["runtime_cutover_authorization_validated"] is True
    assert contract["runtime_cutover_authorized"] is False
    assert contract["runtime_cutover_executed"] is False
...
def test_wrong_branch_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_NEXT_MOVE_DRIFT"):
        validation.run(reports_root=reports)
...
def test_empty_input_bindings_fail_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_INPUT_BINDINGS_EMPTY"):
        validation.run(reports_root=reports)
...
def test_outcome_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    path = reports / auth.OUTPUTS["packet_contract"]
    payload = _load(path)
    payload["selected_outcome"] = "RUNTIME_CUTOVER_AUTHORIZED"
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_PACKET_OUTCOME_DRIFT"):
        validation.run(reports_root=reports)
...
@pytest.mark.parametrize(
    "field, value, reason",
    [
        ("runtime_cutover_authorized", True, "RC_B04R6_CUTOVER_AUTH_VAL_RUNTIME_CUTOVER_AUTHORIZED"),
        ("runtime_cutover_executed", True, "RC_B04R6_CUTOVER_AUTH_VAL_RUNTIME_CUTOVER_EXECUTED"),
        ("activation_cutover_executed", True, "RC_B04R6_CUTOVER_AUTH_VAL_ACTIVATION_CUTOVER_EXECUTED"),
        ("r6_open", True, "RC_B04R6_CUTOVER_AUTH_VAL_R6_OPEN_DRIFT"),
        ("package_promotion_authorized", True, "RC_B04R6_CUTOVER_AUTH_VAL_PACKAGE_PROMOTION_DRIFT"),
        ("commercial_activation_claim_authorized", True, "RC_B04R6_CUTOVER_AUTH_VAL_COMMERCIAL_CLAIM_DRIFT"),
        ("truth_engine_law_changed", True, "RC_B04R6_CUTOVER_AUTH_VAL_TRUTH_ENGINE_MUTATION"),
        ("trust_zone_law_changed", True, "RC_B04R6_CUTOVER_AUTH_VAL_TRUST_ZONE_MUTATION"),
    ],
)
def test_authority_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, field: str, value: object, reason: str) -> None:
    reports = _run_authoring_only(tmp_path, monkeypatch)
    path = reports / auth.OUTPUTS["packet_contract"]
    payload = _load(path)
    payload[field] = value
...
def test_plain_r6_open_claim_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_R6_OPEN_DRIFT"):
        validation.run(reports_root=reports)


def test_claim_bearing_positive_token_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_authoring_only(tmp_path, monkeypatch)
    path = reports / auth.OUTPUTS["commercial_claim_boundary"]
    payload = _load(path)
    payload["commercial_claims"] = "COMMERCIAL_ACTIVATION"
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_CLAIM_TOKEN_DRIFT"):
        validation.run(reports_root=reports)
...
def test_prep_only_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="RC_B04R6_CUTOVER_AUTH_VAL_PREP_ONLY_DRIFT"):
        validation.run(reports_root=reports)
