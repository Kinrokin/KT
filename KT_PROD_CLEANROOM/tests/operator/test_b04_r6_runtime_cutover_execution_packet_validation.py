...
VALIDATION_MAIN_HEAD = "1e74e888f8f47f783b00e879f05cac4fd5b6d1f5"


def _load_packet_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_runtime_cutover_execution_packet.py")
    spec = importlib.util.spec_from_file_location("b04_r6_runtime_cutover_execution_packet_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load runtime cutover execution packet helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("runtime_cutover_execution_packet_validation")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_runtime_cutover_execution_validation_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
...
def test_validation_selects_runtime_cutover_next(outputs: Path) -> None:
...
def test_validation_marks_packet_validated_but_no_runtime_execution(outputs: Path) -> None:
    contract = _contract(outputs)
    assert contract["runtime_cutover_execution_packet_authored"] is True
    assert contract["runtime_cutover_execution_packet_validated"] is True
...
def test_packet_outcome_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = _load(path)
    payload["selected_outcome"] = "B04_R6_RUNTIME_CUTOVER_EXECUTION_PACKET_DEFERRED__NAMED_PACKET_DEFECT_REMAINS"
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
...
def test_input_already_claims_validation_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    path = reports / packet.OUTPUTS["packet_contract"]
    payload = _load(path)
    payload["runtime_cutover_execution_packet_validated"] = True
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="PACKET_OUTCOME_DRIFT"):
        validation.run(reports_root=reports)
...
def test_prep_only_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_packet_only(tmp_path, monkeypatch)
    path = reports / packet.OUTPUTS["runtime_cutover_run_result_schema_prep_only"]
    payload = _load(path)
...
def test_text_artifact_forbidden_claim_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    path.write_text(text + "\nGLOBAL RUNTIME SURFACE AUTHORIZED\n", encoding="utf-8")
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure, match="GLOBAL_SURFACE_DRIFT"):
        validation.run(reports_root=reports)
