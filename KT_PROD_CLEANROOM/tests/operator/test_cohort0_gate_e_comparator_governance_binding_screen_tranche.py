...
from tools.operator import cohort0_gate_e_comparator_governance_binding_screen_tranche as tranche
...
def test_gate_e_binding_screen_confirms_bundle_and_authorizes_reconvening_main_screen(tmp_path: Path) -> None:
    ...
    _write_json(
        reports / "cohort0_gate_e_admissibility_screen_receipt.json",
        {
            ...
            "named_bounded_defect_id": tranche.binding_packet_tranche.PREDICATE_GATE_E_BINDING,
            "gate_e_open": False,
        },
    )
    _write_json(
        reports / tranche.binding_packet_tranche.OUTPUT_PACKET,
        {
            "status": "PASS",
            "subject_head": subject_head,
            tranche.binding_packet_tranche.PREDICATE_GATE_E_BINDING: True,
        },
    )
    _write_json(
        reports / tranche.binding_packet_tranche.OUTPUT_RECEIPT,
        {
            "status": "PASS",
            "subject_head": subject_head,
            tranche.binding_packet_tranche.PREDICATE_GATE_E_BINDING: True,
            tranche.binding_packet_tranche.ARTIFACT_READY_PREDICATE: True,
        },
    )
    ...
    result = tranche.run(
        ...
        binding_packet_path=reports / tranche.binding_packet_tranche.OUTPUT_PACKET,
        binding_receipt_path=reports / tranche.binding_packet_tranche.OUTPUT_RECEIPT,
    ...
    assert result["binding_outcome"] == tranche.OUTCOME_CONFIRMED
    assert result["gate_e_binding_confirmed"] is True
    assert result["gate_e_open"] is False

    receipt = json.loads((reports / tranche.OUTPUT_RECEIPT).read_text(encoding="utf-8"))
    assert receipt["next_lawful_move"] == tranche.NEXT_MOVE_CONFIRMED
