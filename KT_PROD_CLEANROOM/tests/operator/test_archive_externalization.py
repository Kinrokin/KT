...
def test_tracked_active_export_files_excludes_archive_and_noncanonical_paths(tmp_path: Path) -> None:
    ...
    _write_text(root / "KT_ARCHIVE/docs/audit/old.md", "# archived\n")
    ...
    manifest = {
        ...
        "excluded_paths": ["KT_ARCHIVE/**", "KT_PROD_CLEANROOM/reports/**"],
    }
    ...
    assert "KT_ARCHIVE/docs/audit/old.md" not in exported
...
def test_scan_doc_link_failures_detects_archive_and_missing_targets(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    _write_text(root / "README.md", "[ok](docs/guide.md)\n[bad](KT_ARCHIVE/docs/audit/old.md)\n[missing](docs/missing.md)\n")
    _write_text(root / "docs/guide.md", "# guide\n")

    failures = scan_doc_link_failures(root, ["README.md", "docs/guide.md"])

    reasons = {(row["target"], row["reason"]) for row in failures}
    assert ("KT_ARCHIVE/docs/audit/old.md", "archive_link_target") in reasons
