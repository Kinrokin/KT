...
from tools.operator import cohort0_b04_r6_r6_opening_evidence_review_packet_validation as validation


VALIDATION_HEAD = "cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd"
VALIDATION_MAIN_HEAD = "9f982105cfd32f784828caf9a6b679aa502f742d"


def _load_review_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_r6_opening_evidence_review_packet.py")
    spec = importlib.util.spec_from_file_location("b04_r6_opening_evidence_review_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load R6 opening evidence review helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("r6_opening_evidence_review_validation")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_validation_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
    ...
    if filename.endswith(".md"):
        ...
        assert "package-promotion review packet authorship" in text
...
def test_validation_binds_r6_opening_evidence_review_packet(outputs: Path) -> None:
...
def test_validation_selects_package_promotion_review_packet_authorship(outputs: Path) -> None:
    ...
    assert _receipt(outputs)["verdict"] == "R6_OPENING_EVIDENCE_REVIEW_VALIDATED_PACKAGE_PROMOTION_REVIEW_PACKET_NEXT"
...
def test_scorecard_is_validated(outputs: Path) -> None:
    ...
    assert scorecard["opening_result"] == "PASSED"
    assert scorecard["r6_open"] is True
    assert scorecard["route_distribution_health"] == "PASS"
    assert scorecard["fallback_behavior"] == "PASS"
    assert scorecard["package_promotion_review_ready"] is True
...
def test_decision_matrix_is_validated_as_review_packet_next_only(outputs: Path) -> None:
    ...
    assert matrix["package_promotion_review_ready"] is True
    assert matrix["recommendation_is_authority"] is False
    assert matrix["package_promotion_ready"] is False
    assert matrix["commercial_activation_claim_status"] == "BOUNDARY_ONLY"
...
@pytest.mark.parametrize(
    "role",
    [
        "package_promotion_readiness_matrix",
...
def test_review_contracts_are_validated(outputs: Path, role: str) -> None:
    validation_roles = {
        ...
        "package_promotion_blocker_review": "package_promotion_blocker_validation",
    }
...
def test_decision_matrix_recommendation_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = _load(path)
    payload["decision_matrix"]["recommended_next_path"] = "EXTERNAL_AUDIT_DELTA_PACKET_NEXT"
    _write(path, payload)
    _patch_validation_env(monkeypatch, tmp_path)
