...
def test_gate_f_screen_and_review_authorize_and_confirm(tmp_path: Path, monkeypatch) -> None:
    ...
    screen_result = screen_tranche.run(
        reports_root=reports,
        branch_law_packet_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_gate_d_post_clear_branch_law_packet.json",
        supersession_note_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_gate_d_post_clear_supersession_note.json",
        orchestrator_receipt_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_master_orchestrator_receipt.json",
    ...
    assert screen_result["screen_outcome"] == screen_tranche.OUTCOME_AUTHORIZED

    review_result = review_tranche.run(
        reports_root=reports,
        branch_law_packet_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_gate_d_post_clear_branch_law_packet.json",
        supersession_note_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_gate_d_post_clear_supersession_note.json",
        orchestrator_receipt_path=tmp_path / "KT_PROD_CLEANROOM" / "reports" / "cohort0_successor_master_orchestrator_receipt.json",
    ...
    assert review_result["review_outcome"] == review_tranche.OUTCOME_CONFIRMED

    review_receipt = _load(reports / review_tranche.OUTPUT_RECEIPT)
    assert review_receipt["gate_f_narrow_wedge_confirmed"] is True
    assert review_receipt["gate_f_open"] is False
