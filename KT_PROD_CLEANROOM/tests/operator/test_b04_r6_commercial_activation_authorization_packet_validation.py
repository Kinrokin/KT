...
from tools.operator import cohort0_b04_r6_commercial_activation_authorization_packet_validation as validation


VALIDATION_HEAD = "edededededededededededededededededededed"
VALIDATION_MAIN_HEAD = "39d493dfb63f1b35f6325c93d39bbc846ce32a27"


def _load_authorization_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_commercial_activation_authorization_packet.py")
    spec = importlib.util.spec_from_file_location("b04_r6_commercial_activation_auth_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load commercial activation authorization helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("commercial_activation_authorization_validation")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_validation_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
    ...
    if filename.endswith(".md"):
        ...
        assert "commercial activation authorization validation" in text
        assert "commercial activation is not executed" in text
    else:
        payload = _load(path)
...
def test_authorization_claim_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_authorization(tmp_path, monkeypatch)
    ...
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_COMMERCIAL_CLAIM_DRIFT"
...
def test_authorization_execution_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_EXECUTION_DRIFT"
...
def test_benchmark_authority_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_BENCHMARK_AUTHORITY_DRIFT"
...
def test_claim_token_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_CLAIM_TOKEN_DRIFT"


def test_input_binding_hash_mismatch_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_authorization(tmp_path, monkeypatch)
    auth_contract_path = reports / authorization.OUTPUTS["authorization_contract"]
    auth_contract = _load(auth_contract_path)
    first_binding = auth_contract["input_bindings"][0]
    target = tmp_path / first_binding["path"]
    target.write_text(target.read_text(encoding="utf-8") + "\n", encoding="utf-8")
    _patch_validation_env(monkeypatch, tmp_path)
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_INPUT_HASH_MISMATCH"
...
def test_git_object_preoverwrite_binding_hash_mismatch_fails_closed(
    ...
    auth_contract_path = reports / authorization.OUTPUTS["authorization_contract"]
    auth_contract = _load(auth_contract_path)
    auth_contract["input_bindings"].append(
        {
    ...
    auth_contract["binding_hashes"]["pre_overwrite_probe_hash"] = "0" * 64
    _write(auth_contract_path, auth_contract)
...
def test_main_replay_requires_head_equal_origin_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(validation.LaneFailure) as excinfo:
        validation.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_AUTH_VAL_NEXT_MOVE_DRIFT"
