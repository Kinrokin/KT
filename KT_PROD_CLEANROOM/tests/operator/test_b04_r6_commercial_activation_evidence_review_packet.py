...
REVIEW_MAIN_HEAD = "e6e93d0a08662fb1638f27caf20ea46b69c808a0"


def _load_activation_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_commercial_activation.py")
    spec = importlib.util.spec_from_file_location("b04_r6_commercial_activation_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load commercial activation helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("commercial_activation_evidence_review")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_required_review_outputs_exist_and_parse(outputs: Path, filename: str) -> None:
    ...
    if filename.endswith(".md"):
        ...
        assert "commercial activation evidence review packet" in text
...
def test_review_binds_commercial_activation(outputs: Path) -> None:
    contract = _contract(outputs)
    assert contract["authoritative_lane"] == review.AUTHORITATIVE_LANE
    assert contract["previous_authoritative_lane"] == activation.AUTHORITATIVE_LANE
    assert contract["predecessor_outcome"] == activation.SELECTED_OUTCOME
    assert contract["previous_next_lawful_move"] == activation.NEXT_LAWFUL_MOVE
    assert contract["binding_hashes"]["activation_contract_hash"]
    assert contract["binding_hashes"]["activation_receipt_hash"]
...
def test_claim_readiness_routes_to_follow_up_audit_first(outputs: Path) -> None:
    matrix = _payload(outputs, "claim_authorization_readiness_matrix")
    assert matrix["claim_authorization_ready"] is False
    assert matrix["readiness_status"] == "FOLLOW_UP_AUDIT_REVIEW_FIRST"
    assert _contract(outputs)["recommended_next_path"] == review.RECOMMENDED_NEXT_PATH
...
def test_commercial_claim_authorization_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_activation(tmp_path, monkeypatch)
    contract_path = reports / activation.OUTPUTS["activation_contract"]
    ...
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_EVID_REVIEW_CLAIM_DRIFT"
...
def test_claim_text_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_EVID_REVIEW_CLAIM_TOKEN_DRIFT"


def test_follow_up_audit_readiness_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_activation(tmp_path, monkeypatch)
    contract_path = reports / activation.OUTPUTS["activation_contract"]
    payload = _load(contract_path)
    ...
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_EVID_REVIEW_AUDIT_READINESS_DRIFT"


def test_prep_only_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_activation(tmp_path, monkeypatch)
    path = reports / activation.OUTPUTS["follow_up_audit_readiness_packet_prep_only_draft"]
    payload = _load(path)
    ...
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_EVID_REVIEW_PREP_ONLY_DRIFT"
...
def test_main_replay_requires_head_equal_origin_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
    assert excinfo.value.code == "RC_B04R6_COMMERCIAL_ACTIVATION_EVID_REVIEW_NEXT_MOVE_DRIFT"
...
def test_malformed_self_replay_next_move_receipt_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = _load(path)
    payload["previous_next_lawful_move"] = "AUTHOR_BAD_PACKET"
    _write(path, payload)
    _patch_review_env(monkeypatch, tmp_path, branch=f"{review.REPLAY_BRANCH_PREFIX}-test")
    with pytest.raises(review.LaneFailure) as excinfo:
        review.run(reports_root=reports)
...
def test_all_json_outputs_record_branch_head_as_current_git_head(outputs: Path, role: str) -> None:
    payload = _payload(outputs, role)
    assert payload["current_git_head"] == REVIEW_HEAD
    assert payload["current_branch_head"] == REVIEW_HEAD
