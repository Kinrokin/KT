...
def test_documentary_carrier_mismatch_fails_closed_in_both_consumers() -> None:
    root = _repo_root()
    ...
    assert final_guard["documentary_carrier_attempt"]["failure_reason"] == "DOCUMENTARY_CARRIER_ONLY_SUBJECT_HEAD_MISMATCH"
    assert w3_guard["documentary_carrier_attempt"]["failure_reason"] == "DOCUMENTARY_CARRIER_ONLY_SUBJECT_HEAD_MISMATCH"
    assert final_guard["tracked_t11_contract"]["failure_reason"] == "SUBJECT_HEAD_MISMATCH"
    assert w3_guard["tracked_t11_contract"]["failure_reason"] == "SUBJECT_HEAD_MISMATCH"
    ...
    assert final_guard["shared_guard_helper_owner_ref"] == "KT_PROD_CLEANROOM/tools/operator/benchmark_constitution_validate.py"
    assert w3_guard["shared_guard_helper_owner_ref"] == "KT_PROD_CLEANROOM/tools/operator/benchmark_constitution_validate.py"


def test_w3_cli_emits_t12_receipt_with_explicit_opt_in_only(tmp_path: Path) -> None:
    ...
    t12_receipt_path = tmp_path / "documentary_carrier_fail_closed_consumer_enforcement_receipt.json"
    ...
    proc = subprocess.run(
        [
            ...
            "--emit-documentary-carrier-fail-closed-consumer-enforcement-receipt",
            "--documentary-carrier-fail-closed-consumer-enforcement-output",
            str(t12_receipt_path),
        ],
        cwd=str(root),
        env=env,
        text=True,
    ...
    receipt = json.loads(t12_receipt_path.read_text(encoding="utf-8"))
    assert receipt["status"] == "PASS"
    assert receipt["receipt_role"] == "COUNTED_T12_DOCUMENTARY_CARRIER_FAIL_CLOSED_CONSUMER_ENFORCEMENT_ARTIFACT_ONLY"
    assert receipt["final_current_head_consumer_guard"]["status"] == "PASS"
    assert receipt["w3_consumer_guard"]["status"] == "PASS"
