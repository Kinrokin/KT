...
AUTH_HEAD = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
AUTH_MAIN_HEAD = "90aa176ca029d45811bbb653e085a49bbf74bd34"


def _load_validation_helpers():
    helper_path = Path(__file__).with_name("test_b04_r6_runtime_cutover_review_packet_validation.py")
    spec = importlib.util.spec_from_file_location("b04_r6_runtime_cutover_review_validation_helpers", helper_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load runtime cutover review validation helpers")
...
def outputs(tmp_path_factory: pytest.TempPathFactory) -> Path:
    tmp_path = tmp_path_factory.mktemp("runtime_cutover_authorization_packet")
    monkeypatch = pytest.MonkeyPatch()
    try:
...
GUARD_FALSE_FIELDS = [
    "runtime_cutover_authorization_validated",
...
def test_packet_selects_authorization_validation_next(outputs: Path) -> None:
    ...
    assert _receipt(outputs)["verdict"] == "BOUND_FOR_RUNTIME_CUTOVER_AUTHORIZATION_VALIDATION_ONLY"
...
@pytest.mark.parametrize("row_index", range(0, 170))
...
def test_allowed_and_excluded_surfaces_are_bounded(outputs: Path) -> None:
    ...
    assert "B04_R6_ROUTER_DECISION_SURFACE_BOUNDED" in allowed
...
def test_pipeline_board_marks_authorization_validation_next(outputs: Path) -> None:
    ...
    assert board["runtime_cutover_authorization_packet"] == "BOUND"
    assert board["runtime_cutover_authorization_validation"] == "NEXT"
...
def test_next_lawful_move_is_authorization_validation(outputs: Path) -> None:
    assert _next(outputs)["next_lawful_move"] == "VALIDATE_B04_R6_RUNTIME_CUTOVER_AUTHORIZATION_PACKET"
...
def test_previous_outcome_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = _load(path)
    payload["selected_outcome"] = "B04_R6_RUNTIME_CUTOVER_REVIEW_DEFERRED__NAMED_REVIEW_DEFECT_REMAINS"
    _write(path, payload)
    _patch_auth_env(monkeypatch, tmp_path)
...
def test_previous_next_move_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = _load(path)
    payload["next_lawful_move"] = "AUTHOR_B04_R6_RUNTIME_CUTOVER_EXECUTION_PACKET"
    _write(path, payload)
    _patch_auth_env(monkeypatch, tmp_path)
...
def test_runtime_execution_drift_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_predecessor(tmp_path, monkeypatch)
    path = reports / auth.review_validation.OUTPUTS["validation_contract"]
    payload = _load(path)
    payload["runtime_cutover_executed"] = True
    _write(path, payload)
    _patch_auth_env(monkeypatch, tmp_path)
    with pytest.raises(auth.LaneFailure, match="RUNTIME_CUTOVER_EXECUTED"):
        auth.run(reports_root=reports)


def test_runtime_execution_non_false_fails_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reports = _run_predecessor(tmp_path, monkeypatch)
    path = reports / auth.review_validation.OUTPUTS["validation_contract"]
    payload = _load(path)
    payload["runtime_cutover_executed"] = ""
    _write(path, payload)
    _patch_auth_env(monkeypatch, tmp_path)
    with pytest.raises(auth.LaneFailure, match="RUNTIME_CUTOVER_EXECUTED"):
        auth.run(reports_root=reports)
