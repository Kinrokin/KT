...
- `python -m tools.operator.kt_cli status`

2) Certify (sweep harness):
- `python -m tools.operator.kt_cli certify --profile v1 --lane ci_sim`
- `python -m tools.operator.kt_cli certify --profile v1 --lane canonical_hmac` (requires HMAC keys)

3) Report (human-facing summary of a run):
- `python -m tools.operator.kt_cli report --run <run_dir>`
