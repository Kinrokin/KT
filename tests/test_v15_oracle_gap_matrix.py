...
def test_oracle_gap_matrix_is_v15_bound_and_legal():
    rows = read_jsonl("admission/oracle_gap_matrix.jsonl")
    assert len(rows) == 28
    assert all(row["schema_id"] == "kt.oracle_gap_row.v1" for row in rows)
    ...
    assert all("oracle_correct" not in row["pre_generation_features"] for row in rows)
